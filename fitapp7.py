...
class CalorieCalculatorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Fitness Calorie Calculator")
    ...
    def add_meal(self):
        ...
        new_meal = MealEntry(self.meal_frame, df, self.refresh_total_nutrition, len(self.meals) + 1)
    ...
    def calculate_total_nutrition(self):
        ...
        self.food_items = [f"{food}: {str(qty) + 'g' if 'per 100g' in unit_mapping[food] else str(int(qty))}" for food, qty in combined_items.items()]
    ...
    def update_total_nutrition_display(self):
        ...
        self.total_text.insert(tk.END, f"Total Protein: {self.total_nutrients['Protein']:.1f} g\n")
    ...
    def update_journal(self):
        ...
        for meal in self.meals:
            for food_info in meal.food_list:
                ...
                quantity_display = f"{qty}g" if 'per 100g' in food_data['Unit'] else f"{int(qty)}"
    ...
    def refresh_journal_display(self):
        ...
        if not today_entries.empty:
            ...
            for index, row in today_entries.iterrows():
                ...
                self.display_journal_text.insert(tk.END, f"{food_display:<15} {row['Quantity']:<15} {row['Calories']:<15.1f} {row['Protein']:<15.1f} {row['Carbs']:<15.1f} {row['Fats']:<15.1f}\n")
            ...
            self.display_journal_text.insert(tk.END, f"{'':<15} {'':<15} {total_calories:<15.1f} {total_protein:<15.1f} {total_carbs:<15.1f} {total_fats:<15.1f}\n")
        else:
...
class MealEntry:
    ...
    def setup_widgets(self, meal_number):
        ...
        for food_type in ['Carbs', 'Protein', 'Other']:
            ...
            #qty_entry.insert(0, "Enter quantity, e.g., rice: 150")
        ...
        try:
            ...
            for i, item in enumerate(self.food_list):
                if item.startswith(food_name + ':'):
                    old_qty = int(item.split(': ')[1].replace('g', ''))
    ...
    def update_nutrition_display(self):
        ...
        self.nutrition_text.insert(tk.END, f"Protein: {self.nutrients['Protein']:.1f} g\n")
